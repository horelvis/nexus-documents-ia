...
Removed 2026-05-08. All authenticated users see every document; there is no per-document role filter
on Weaviate queries. The `roles` property and the `_roles_filter()` helper in `weaviate_service.py`
were dropped together (PR #2, merge `dc718acd`). Admin-only endpoints gate on `User.is_superuser`
via `Depends(require_superuser)` instead. See [ACL_SYSTEM.md](ACL_SYSTEM.md).
...
2. **Check PostgreSQL ↔ Weaviate consistency** — if the document was reindexed but Weaviate still has stale chunks, retrieval may surface obsolete content. Confirm both stores agree:
   ```sql
   SELECT id, title, indexing_status, chunk_count FROM indexed_documents WHERE id = '<uuid>';
   ```
   _ACL filtering was removed 2026-05-08; all authenticated users see every document._
