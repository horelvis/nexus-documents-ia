...
docker compose exec db psql -U postgres -d doc_management -c "UPDATE alembic_version SET version_num = 'add_sub_fields_001' WHERE version_num = 'add_doc_categorization';"

echo ""
echo "2️⃣ Current migration status:"
docker compose exec db psql -U postgres -d doc_management -c "SELECT version_num FROM alembic_version;"
