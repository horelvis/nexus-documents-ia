...
Version 1.1 - January 2026 (Added model security validation)
"""
...
# Trusted model organizations - models from these orgs can use trust_remote_code
...
def is_trusted_model(model_name: str) -> bool:
    ...
    """
    return any(source.lower() in model_name.lower() for source in TRUSTED_MODEL_SOURCES)


def safe_trust_remote_code(model_name: str, requested: bool = True) -> bool:
    """
    Validate trust_remote_code request against trusted sources.

    Args:
        model_name: Model name to check
        requested: Whether trust_remote_code was requested

    Returns:
        Validated trust_remote_code value
    """
    if not requested:
        return False

    if is_trusted_model(model_name):
        logger.info(f"✅ Model '{model_name}' is from trusted source, allowing trust_remote_code=True")
        return True

    logger.warning(
        f"⚠️ SECURITY: Model '{model_name}' is NOT from a trusted source. "
        f"Disabling trust_remote_code. Trusted sources: {TRUSTED_MODEL_SOURCES}"
    )
    return False
...
def setup_model_and_tokenizer(model_name: str, use_4bit: bool = True):
    """Load model with LoRA configuration and security validation."""

    logger.info(f"Loading model: {model_name}")

    # Validate trust_remote_code for this model
    trust_code = safe_trust_remote_code(model_name, requested=True)

    # Tokenizer with security-validated trust_remote_code
...
def merge_and_save(
    ...
    # Validate trust_remote_code for security
    trust_code = safe_trust_remote_code(base_model_name, requested=True)
