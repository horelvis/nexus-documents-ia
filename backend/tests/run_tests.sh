...
if [ ! -f "docker-compose.test.yml" ]; then
    error "No se encontró docker-compose.test.yml. Ejecuta desde backend/docker/"
    exit 1
fi
...
timeout 600s docker compose -f docker-compose.test.yml up --abort-on-container-exit --exit-code-from test-api
...
if [ $exit_code -eq 0 ]; then
    ...
    docker compose -f docker-compose.test.yml logs test-db | tail -20
    docker compose -f docker-compose.test.yml logs test-redis | tail -10
    docker compose -f docker-compose.test.yml logs test-qdrant | tail -10
fi
