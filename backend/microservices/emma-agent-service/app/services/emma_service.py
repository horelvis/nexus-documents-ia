...
Responde SOLO una palabra:"""
    ...
    async def _get_enriched_user_context(
        ...
        if request_context:
            ...
            request_context = upload_context_service.hydrate_context(request_context)
            ...
            if request_context.get("indexed_document_ids"):
                user_context["indexed_document_ids"] = request_context["indexed_document_ids"]
            if request_context.get("focus_document"):
                user_context["focus_document"] = request_context["focus_document"]
            if request_context.get("uploaded_texts"):
                user_context["uploaded_texts"] = request_context.get("uploaded_texts", [])
    ...
    def _apply_uploaded_context(self, query_text: str, user_context: Optional[Dict[str, Any]]) -> str:
        if not user_context:
            return query_text

        parts = [query_text]

        # Inject indexed document context so the LLM knows which document is in scope
        doc_id = user_context.get("document_id")
        indexed_ids = user_context.get("indexed_document_ids") or []
        doc_title = user_context.get("document_title")
        if doc_id or indexed_ids:
            context_lines = ["\n\n[Contexto del documento seleccionado]"]
            if doc_id:
                context_lines.append(f"document_id: {doc_id}")
            if indexed_ids:
                context_lines.append(f"indexed_document_ids: {', '.join(indexed_ids)}")
            if doc_title:
                context_lines.append(f"título: {doc_title}")
            parts.append("\n".join(context_lines))

        # Inject uploaded (non-indexed) document texts
        uploaded_texts = user_context.get("uploaded_texts") or []
        if uploaded_texts:
            parts.append("\n\nContenido de documentos subidos (no indexados):")
        ...
        if conversational_response:
            ...
            try:
                await self._store_conversation(
                    ...
                    user_id=user_id,
                    tools_used=[],
                    document_context=self._extract_document_context(query.context)
                )
            except Exception as e:
    ...
    async def _execute_with_emma(
        ...
        # Set execution context for tools (thread-safe via contextvars)
        document_id = (query.context or {}).get("document_id")
        ...
        try:
            ...
            query_for_llm = self._apply_uploaded_context(query.query, user_context)
    ...
    async def _execute_with_langgraph(
        ...
        try:
            ...
            if result.fast_path:
                decision_path.append("fast_path")
            if result.domains:
    ...
    def _extract_document_context(self, query_context: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """Extract document context fields from query context for persistence."""
        if not query_context:
            return None
        doc_ctx = {}
        for key in ("document_id", "uploaded_file_ids", "indexed_document_ids"):
            val = query_context.get(key)
            if val:
                doc_ctx[key] = val
        return doc_ctx if doc_ctx else None
    ...
    async def _store_conversation(
        ...
        """Store conversation in Memory Protocol and PostgreSQL persistence."""
        try:
            # Build active_documents list for Redis memory
            active_docs: List[str] = []
            if document_context:
                if document_context.get("document_id"):
                    active_docs.append(document_context["document_id"])
                for doc_id in (document_context.get("indexed_document_ids") or []):
                    if doc_id and doc_id not in active_docs:
                        active_docs.append(doc_id)
                for file_id in (document_context.get("uploaded_file_ids") or []):
                    if file_id and file_id not in active_docs:
            ...
            await self._memory.add_exchange(
                ...
                user_id=user_id,
                active_documents=active_docs if active_docs else None,
