    ...
    async def initialize(self) -> None:
        ...
        async with session_factory() as session:
            try:
                ...
                logger.warning(f"MemoRAG vector extension init failed: {e}")
            ...
            try:
                ...
                logger.warning(f"MemoRAG table init failed: {e}")
            ...
            try:
                ...
                logger.warning(f"MemoRAG index init failed: {e}")

            try:
                await session.commit()
            except Exception as e:
                logger.warning(f"MemoRAG init commit failed: {e}")
        ...
        async with session_factory() as session:
            ...
            for idx, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
                ...
                await session.execute(text(
                    ...
                    VALUES (:tenant_id, :document_id, :chunk_index, :content, :content_hash, :metadata, :embedding::vector, :memory_version)
                    """
                ), {
                    "tenant_id": tenant_id,
