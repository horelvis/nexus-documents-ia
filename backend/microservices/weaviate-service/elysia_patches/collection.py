...
async def _evaluate_field_statistics(
    ...
    # FIX: Use empty list instead of None to avoid GRPC proto error
    out["groups"] = []
...
async def preprocess_async(
    ...
    if client_manager is None:
        # FIX: Pass local Weaviate parameters to ClientManager
        client_manager = ClientManager(
            wcd_url=settings.WCD_URL,
            wcd_api_key=settings.WCD_API_KEY,
            weaviate_is_local=settings.WEAVIATE_IS_LOCAL,
            weaviate_is_custom=settings.WEAVIATE_IS_CUSTOM,
            local_weaviate_port=settings.LOCAL_WEAVIATE_PORT,
            local_weaviate_grpc_port=settings.LOCAL_WEAVIATE_GRPC_PORT,
            custom_http_host=settings.CUSTOM_HTTP_HOST,
            custom_http_port=settings.CUSTOM_HTTP_PORT,
            custom_http_secure=settings.CUSTOM_HTTP_SECURE,
            custom_grpc_host=settings.CUSTOM_GRPC_HOST,
            custom_grpc_port=settings.CUSTOM_GRPC_PORT,
            custom_grpc_secure=settings.CUSTOM_GRPC_SECURE,
            settings=settings,
        )
...
async def preprocessed_collection_exists_async(
    ...
    """
    if client_manager is None:
        # FIX: Use environment_settings for local Weaviate support
        client_manager = ClientManager(
            wcd_url=environment_settings.WCD_URL,
            wcd_api_key=environment_settings.WCD_API_KEY,
            weaviate_is_local=environment_settings.WEAVIATE_IS_LOCAL,
            weaviate_is_custom=environment_settings.WEAVIATE_IS_CUSTOM,
            local_weaviate_port=environment_settings.LOCAL_WEAVIATE_PORT,
            local_weaviate_grpc_port=environment_settings.LOCAL_WEAVIATE_GRPC_PORT,
