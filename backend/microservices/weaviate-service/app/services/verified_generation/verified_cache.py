...
def get_verified_cache() -> VerifiedContextCache:
    ...
    if _verified_cache is None:
        # Use configured TTL from settings if available
        ttl = getattr(settings, 'verified_cache_ttl_seconds', VerifiedContextCache.DEFAULT_TTL_SECONDS)
        _verified_cache = VerifiedContextCache(ttl_seconds=ttl)
