...
Usage:
    ...
    job_id = await job_manager.start_job("sync", sync_coro, params)
...
class JobManager:
    ...
    async def start_job(
        ...
        """
        Start an async job in the background.

        Args:
            job_type: Type of job (e.g., "sync", "index-pending")
            coro_func: Async function to run. Must accept (job_state, **params).
            params: Parameters passed to coro_func.
