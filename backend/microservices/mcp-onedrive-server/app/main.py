"""
MCP OneDrive Server Entry Point.
...
- /oauth/authorize - Initiate OAuth2 flow (Azure AD)
...
async def run_stdio():
    ...
    logger.info("Starting MCP OneDrive Server in STDIO mode")
...
async def run_http(host: str = "0.0.0.0", port: int = 8000):
    ...
    logger.info(f"Starting MCP OneDrive Server in HTTP mode on {host}:{port}")
    ...
    async def info(request):
        return JSONResponse({
            "name": "mcp-onedrive-server",
            "version": "1.0.0",
            "protocol": "MCP",
            "transport": "HTTP/SSE",
            "oauth_configured": bool(
                settings.ms_oauth_client_id
                and settings.ms_oauth_client_secret
            ),
            "tools": [
    ...
    async def oauth_authorize(request: Request):
        ...
        try:
            # If connector already has a microsoft_email, use it as login_hint
            login_hint = None
            try:
...
<p style="color:#666;margin-bottom:1.5rem">{display_error}</p>
...
<button onclick="window.close()">Cerrar</button></div></body></html>"""
    ...
    async def list_folders(request: Request):
        ...
        try:
            ...
            drive_base = f"https://graph.microsoft.com/v1.0/drives/{config.drive_id}" if config.drive_id else "https://graph.microsoft.com/v1.0/me/drive"

            if parent_id == "root":
                url = f"{drive_base}/root/children"
            else:
                url = f"{drive_base}/items/{parent_id}/children"
            ...
            folders = [
                ...
                if "folder" in f or True  # $filter already handles this
            ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MCP OneDrive Server - Document management for OneDrive for Business"
    )
    parser.add_argument(
        "--http",
        action="store_true",
