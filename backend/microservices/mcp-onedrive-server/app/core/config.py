...
OneDrive uses OAuth2 via Azure AD (MSAL). Tokens are stored
...
class OneDriveInstanceConfig(BaseModel):
    """Configuration for a single OneDrive connector instance."""
    ...
    # OneDrive settings
    drive_id: Optional[str] = Field(default=None, description="Specific drive ID (default: user's drive)")
    folder_id: Optional[str] = Field(default=None, description="Root folder item ID to sync")
    ...
    microsoft_email: Optional[str] = Field(default=None, description="Microsoft account email")
...
class Settings(BaseSettings):
    """MCP OneDrive Server settings."""

    service_name: str = "mcp-onedrive-server"
    ...
    ms_oauth_redirect_uri: str = os.getenv(
        "MS_OAUTH_REDIRECT_URI",
        "https://localhost:3001/connectors/oauth/callback"
    )
    ms_oauth_scopes: str = os.getenv(
        "MS_OAUTH_SCOPES",
    ...
    @property
    def ms_oauth_scopes_list(self) -> List[str]:
        """Parse scopes string into list."""
        return [s.strip() for s in self.ms_oauth_scopes.split() if s.strip()]
...
class ConnectorCache:
    ...
    def __init__(self, ttl_seconds: int = 300):
        self._cache: Dict[str, tuple[OneDriveInstanceConfig, datetime]] = {}
