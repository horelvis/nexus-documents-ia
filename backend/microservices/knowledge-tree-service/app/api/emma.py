    ...
    if is_langgraph_enabled_for_tenant(query.tenant_id):
        ...
        try:
            langgraph_result = await execute_langgraph_query(
                ...
                user_role_ids=None,  # TODO: Add role support
                is_admin=False,
                thread_id=thread_id,
            )
            ...
            return EmmaQueryResponse(
                ...
                sil_answered=langgraph_result.slm_fast_path,
                tokens_saved=0,
    ...
    try:
        ...
        async for event in stream_func(
            ...
            if event_type == "started":
                ...
                yield f"event: delegation\ndata: {json.dumps({'tool': agent_name, 'message': f'Delegando a {agent_display}', 'stage': 'analyzing'})}\n\n"
                ...
                yield f"event: step_complete\ndata: {json.dumps({'tool': agent_name, 'message': f'{agent_name} completado', 'tools_used': tools_used})}\n\n"
