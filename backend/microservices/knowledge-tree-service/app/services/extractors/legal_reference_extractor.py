...
Usage:
    from app.services.extractors import legal_reference_extractor
...
class Modification:
    ...
    modifying_text: str
    modifying_law: Optional[str] = None
...
class LegalReferenceExtractor:
    ...
    ARTICLE_PATTERN = re.compile(
        r'[Aa]rt(?:iculo)?\.?\s*(\d+(?:\.\d+)?(?:\s*(?:bis|ter|quater|quinquies|sexies|septies|octies))?)',
        re.UNICODE,
    )

    ARTICLE_LAW_PATTERN = re.compile(
        r'[Aa]rt(?:iculo)?\.?\s*(\d+(?:\.\d+)?(?:\s*(?:bis|ter|quater))?)'
