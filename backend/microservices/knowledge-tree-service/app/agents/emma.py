"""
Emma - AI Document Management Assistant
...
class Emma:
    """
    Emma - Simplified AI Assistant with SIL Integration.
    ...
    def __init__(
        self,
        config: Optional[EmmaConfig] = None,
        ...
        """
        self.config = config or EmmaConfig()
    ...
    async def initialize(self) -> None:
        """Initialize Emma and its dependencies."""
    ...
    async def execute(
        ...
        Returns:
            EmmaResult with answer and metadata
        """
        ...
        if is_conversational:
            ...
            if self.config.enable_slm_router and self._slm_router and SLM_ROUTER_AVAILABLE:
                logger.info(f"🎯 Emma: Trying SLM Router for query: '{query[:60]}...'")
                ...
                if slm_result:
                    logger.info(f"✅ Emma: SLM Router answered (route={slm_result.metadata.get('toon_route')})")
                    ...
                    logger.info(f"➡️ Emma: SLM Router declined → trying SIL fallback")

            # Step 1b: Try SIL fast path as fallback
            if self.config.enable_sil_fast_path and self._sil:
                logger.info(f"🧠 Emma: Trying SIL fast path for query: '{query[:60]}...'")
                sil_result, sil_doc_count = await self._try_sil_fast_path_with_count(query, context.tenant_id)
                if sil_result:
                    logger.info(f"✅ Emma: SIL fast path answered (tokens_saved={sil_result.tokens_saved})")
                    ...
                    logger.info(f"➡️ Emma: SIL declined (doc_count={sil_doc_count}) → agentic loop")
    ...
    async def execute_stream(
        ...
        for iteration in range(self.config.max_iterations):
            ...
            async for event in self._llm_client.chat_stream(messages, EMMA_TOOLS):
...
Responde de forma completa y útil basándote en esta información. Usa la terminología del contexto ({terms['plural']}) y sugiere qué más podría querer saber el usuario."""
    ...
    async def _try_sil_fast_path_with_count(
        ...
        query: str,
        tenant_id: str,
    ) -> tuple[Optional[EmmaResult], int]:
        """
        Try SIL fast path and return both result and document count.

        Returns:
            Tuple of (EmmaResult or None, document_count)
        """
        try:
            from app.services.sil.schemas import ReasoningType
    ...
    async def _agentic_loop(
        ...
        for iteration in range(self.config.max_iterations):
            # Call LLM
            response = await self._llm_client.chat(messages, EMMA_TOOLS)
...
_emma_instance: Optional[Emma] = None


async def get_emma() -> Emma:
    """Get global Emma instance."""
    global _emma_instance
    if _emma_instance is None:
        _emma_instance = Emma()
        await _emma_instance.initialize()
    return _emma_instance


async def reset_emma() -> None:
    """Reset global Emma instance."""
    global _emma_instance
    _emma_instance = None
