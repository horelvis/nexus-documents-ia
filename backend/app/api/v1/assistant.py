...
async def chat_with_agent(
    ...
    try:
        ...
        # Get tenant ID - prioritize context, fallback to current user
        tenant_id = None
        logger.info(f"Debug tenant resolution: context={request.context}, user.tenant_id={current_user.tenant_id}")
        
        if request.context and request.context.get("tenant_id"):
            tenant_id = request.context.get("tenant_id")
            logger.info(f"Using tenant_id from context: {tenant_id}")
        elif current_user.tenant_id:
            tenant_id = str(current_user.tenant_id)
            logger.info(f"Using tenant_id from current_user: {tenant_id}")
        
        if not tenant_id:
            logger.error(f"No tenant_id available - context: {request.context}, user.tenant_id: {current_user.tenant_id}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Tenant ID is required - user not properly associated with tenant"
            )
...
async def chat_with_agent_stream(
    ...
    # Get tenant ID - prioritize context, fallback to current user
    tenant_id = None
    if chat_request.context and chat_request.context.get("tenant_id"):
        tenant_id = chat_request.context.get("tenant_id")
    elif current_user.tenant_id:
        tenant_id = str(current_user.tenant_id)
...
async def get_welcome_message(
    ...
    try:
        ...
        try:
            ...
            # Request personalized welcome from CrewAI
            system_message = f"""SISTEMA: Genera un mensaje de bienvenida personalizado y breve para {current_user.full_name or 'este usuario'}.
            
            Información del usuario:
            - Tiene {document_count} documentos
            - Tiene {pending_signatures} firmas pendientes
            
            El mensaje debe ser:
            - En español
            - Máximo 2 líneas
            - Cálido y profesional
            - Incluir menciones específicas de sus documentos o firmas si son relevantes
            - Si no tiene documentos ni firmas, dar la bienvenida y explicar brevemente qué puedes hacer
            
            NO uses formato de lista, NO uses saltos de línea múltiples, solo texto natural."""
            
            cag_response = await cag_client.process_with_agent(
                message=system_message,
                context=cag_context,
                agent_type="virtual_assistant",
                tools=[]  # No tools needed for welcome message
            )
            
            if cag_response.get("response") and not cag_response.get("error"):
                welcome_message = cag_response["response"].strip()
        ...
        user_name = current_user.full_name or "Usuario"
        return {
            "message": f"¡Hola {user_name}! Estoy aquí para ayudarte con tus documentos.",
            "personalized": False
        }
