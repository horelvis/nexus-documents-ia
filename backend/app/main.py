"""
Main application module — single-tenant on-premise.

Loads the on-premise deployment module (OIDC auth, connectors, JSONB ACL).
"""
import logging
...
def _load_deployment_module(app):
    """Load the on-premise deployment module (ACL provider, routers, auth)."""
    try:
        ...
        logger.warning(f"Could not load on-premise module: {e}")
