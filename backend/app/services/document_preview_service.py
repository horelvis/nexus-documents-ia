...
from app.services.gotenberg_microservice_client import GotenbergClient
...
class DocumentPreviewService:
    ...
    async def generate_preview(
        ...
        """
        Generate document preview using Gotenberg
        ...
        Returns:
            Dict with generated preview information
        """
        ...
        try:
            ...
            # Check if Gotenberg is available for other formats
            health = await self.gotenberg.health_check()
    ...
    async def _preview_office_with_gotenberg(
        ...
        """Convert Office documents to PDF using Gotenberg"""
        try:
    ...
    async def _preview_text_with_gotenberg(
        ...
        """Convert Markdown/HTML/TXT to PDF using Gotenberg"""
        try:
            file_ext = Path(filename).suffix.lower()
    ...
    async def _preview_existing_pdf(
        ...
        """Process existing PDF to generate thumbnails"""
        logger.info(f"Processing existing PDF {filename}")

        # Try to generate thumbnails, but don't fail if not possible
        thumbnails = []
        try:
        ...
        result = {
            ...
            "pdf_available": True,
            "pdf_storage_path": None,  # Original PDF already in storage
    ...
    async def _generate_pdf_thumbnails(self, document_id: str, pdf_path: str) -> List[str]:
        """Generate PDF thumbnails using pdf2image"""
        try:
    ...
    async def _upload_to_storage(self, local_path: str, storage_path: str) -> bool:
        """Upload file to storage and return success/failure"""
        try:
    ...
    async def _cache_preview_metadata(self, document_id: str, filename: str, metadata: Dict):
        """Cache preview metadata for reuse"""
        try:
            # Save metadata as JSON in storage
            import json
    ...
    async def _get_cached_preview(self, document_id: str, filename: str) -> Optional[Dict]:
        """Search for cached preview"""
        try:
            # Search for metadata in storage
            metadata_storage_path = f"{self.preview_storage_prefix}/{document_id}/{document_id}_metadata.json"

            # Try to download metadata
            metadata_content = await self.storage_service.download_file(metadata_storage_path)

            if metadata_content:
                import json
                metadata = json.loads(metadata_content.decode('utf-8'))

                # Verify cache is still valid (e.g., less than 24 hours)
    ...
    async def _fallback_preview(self, file_path: str, filename: str) -> Dict:
        """Fallback when Gotenberg not available or format not supported"""
        file_ext = Path(filename).suffix.lower()
        file_size = os.path.getsize(file_path)
    ...
    def _get_markdown_css(self) -> str:
        """Basic CSS for Markdown rendering"""
        return """
        body {
    ...
    async def get_preview_info(self, document_id: str) -> Optional[Dict]:
        """Get existing preview info without regenerating"""
