...
2. **Cognitive Search & Insights**
   ...
   - Contextual Q&A, summarization, and anomaly detection powered by vLLM (Qwen3)/OpenAI/Anthropic/Google.
3. **Autonomous Agents & Workflows**
   ...
   - Camunda BPM workflows plus alerting/monitoring for long-running jobs.
...
Data Layer & AI Fabric
  ...
  - Weaviate vector store + Emma AI (Microsoft Agent Framework)
...
Microservices (all gated by `MICROSERVICES_API_KEY`)
  - Weaviate Service (Emma AI + RAG pipeline + vector search)
  - LangExtract / TextExtract (LLM + deterministic extraction)
  - Camunda Service (BPMN workflows)
  - Storage Service (GCS abstraction)
  - Template Editor Service (collaborative template editor)
  - Elasticsearch Service (analytics + hybrid queries)
  - TTS Service, Gotenberg conversion, vLLM Server (GPU inference)
```
...
|---------|------------------|-----------------|
| **Weaviate Service (Emma AI)** | Orchestrates contextual agents (legal, financial, compliance) via Microsoft Agent Framework + RAG pipeline. | Risk matrix for a contract, task breakdown for onboarding packages. |
...
| **Camunda Service** | BPMN workflows for signatures, policy renewals, and advisory tasks. | State machines with SLA timers, compensating actions, alert hooks. |
...
| **Gotenberg & vLLM Hosts** | Document-to-PDF transformation and GPU-accelerated LLM inference (Qwen3-4B). | Print-ready binders, agent reasoning traces. |

### Workflow Spotlight: Advisory & Professional Services
1. **Intake** – Client uploads financials, policies, or legal docs. Camunda opens a BPMN workflow instance tagged to the engagement.
2. **Triage** – LangExtract + TextExtract normalize content; Emma AI applies domain agents (tax, legal, compliance) to surface blockers.
...
- **Consulting / Advisory Firms**
  ...
  - Natural-language requests via Emma: "Show me contracts that still need partner review" triggers semantic search + signature orchestration.
...
- **Emma Intelligence Hub**: A mobile-ready assistant (backed by Weaviate Service + Camunda workflows) that becomes the single entry point for approvals, data requests, and signature flows—embedding biometric approvals (voice + FaceID) to satisfy high-trust processes.
...
- **Near Term (next 2 quarters)**  
  ...
  3. Harden auto-scaling policies for AI microservices (vLLM/Weaviate/Elasticsearch).  
...
- **Consent & lifecycle control**: Storage Service enforces retention rules per dataset, while Camunda workflows model consent capture/renewal with auditable checkpoints.
...
| `workflow-camunda.png` | Camunda BPM workflow timeline for advisory engagement (intake → approval). |
